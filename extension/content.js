(() => {
  ...
  window.addEventListener('blur', () => {
    if (held || recording) { held = false; send('cancel-recording'); }
  });
  window.addEventListener('pagehide', () => { if (held || recording) send('cancel-recording'); });
  chrome.runtime.onMessage.addListener(message => {
    ...
    overlay.transcript.textContent = message.transcript ? `指示：${message.transcript}` : '';
    overlay.transcript.hidden = !message.transcript;
